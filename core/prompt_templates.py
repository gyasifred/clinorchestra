...
2. **If data NOT documented**: Use RAG guidelines to write what SHOULD BE DONE (recommended guidelines, monitoring schedule, what to assess)
3. **If data IS documented**: Extract it AND enhance by comparing with RAG guidelines, adding guideline-recommended areas not yet addressed
...
7. Use RAG-retrieved guidelines for clinical interpretation (don't restate criteria)
...
2. CLINICAL SYMPTOMS - TEMPORAL (NEVER LEAVE EMPTY):
   ...
   - **If symptoms NOT documented**: Write guideline-based recommendations using RAG: "No specific symptoms documented in the record. Per [GUIDELINE], comprehensive symptom assessment should include: (1) GI symptoms: frequency/severity of vomiting, diarrhea, reflux, abdominal pain; (2) Systemic: fever patterns, fatigue, irritability; (3) Feeding: appetite changes, food refusal; (4) Functional: activity level, developmental regression. Recommend systematic symptom review at each encounter with [INTERVAL] monitoring."
   - **If symptoms ARE documented**: Also compare with RAG guidelines and add any guideline-recommended symptom categories not yet assessed
   
3. **GROWTH & ANTHROPOMETRICS - TEMPORAL CAPTURE (NEVER LEAVE EMPTY)**:
   - **If measurements ARE documented**: Extract ALL with DATES: "Weight 12.5 kg on 1/15 (5th %ile, z-score -1.8)", calculate TRENDS (absolute change, %, rate, velocity, trajectory), describe pattern, interpret using RAG guidelines. Then compare with RAG to identify missing parameters (e.g., "Head circumference not documented - per [GUIDELINE], should be measured at each visit for children <36 months")
   - **If measurements NOT documented**: Use RAG guidelines to write comprehensive recommendations: "Growth parameters not documented in this encounter. Per [GUIDELINE NAME] for [diagnosis], baseline anthropometric assessment should include: Weight-for-age with percentile/z-score, Height/length-for-age with percentile/z-score, BMI-for-age or weight-for-length with percentile/z-score, Head circumference (if <36 months), Mid-upper arm circumference (if malnutrition suspected). Recommend obtaining complete growth assessment with serial measurements at [INTERVAL] to establish growth trajectory and percentile tracking patterns."

4. **PHYSICAL EXAM - TEMPORAL (NEVER LEAVE EMPTY)**:
   - **If exam documented**: Describe findings with dates, if serial exams show progression, quote exact findings, interpret using RAG. Compare with RAG to identify missing exam components
   - **If exam NOT documented or incomplete**: Use RAG guidelines: "Physical examination not fully documented. Per [GUIDELINE] for malnutrition assessment, comprehensive exam should evaluate: (1) General appearance: alertness, hydration status; (2) Muscle wasting: temporalis, deltoids, quadriceps, interosseous; (3) Subcutaneous fat loss: orbital, triceps, subscapular; (4) Edema: periorbital, pedal, sacral; (5) Skin/hair changes; (6) Growth chart plotting. For serial assessment, recommend documenting these findings at each encounter to track progression or improvement."
...
7. **LABS & SCREENING - TEMPORAL (NEVER LEAVE EMPTY)**:
   - **If labs ARE documented**: Extract ALL with DATES: "Albumin: 3.8 on 1/15, 3.5 on 2/15, 3.2 on 3/15", describe TRENDS: "Albumin declining 16% over 2 months", interpret using RAG. Compare with RAG to identify missing lab work
   - **If labs NOT documented AND diagnosis is MALNUTRITION PRESENT**: Use RAG guidelines to recommend: "Laboratory investigations not documented. Per [GUIDELINE] for [severity] malnutrition, comprehensive metabolic assessment should include: BASELINE PANEL: (1) Complete blood count (assess anemia, immune function); (2) Comprehensive metabolic panel (electrolytes, renal, hepatic function); (3) Albumin and prealbumin (protein status - note limitations for acute illness); (4) Vitamins/minerals: Iron studies, Vitamin D 25-OH, Zinc, B12, folate; (5) Other: Thyroid function if growth faltering, Celiac screening if indicated. SERIAL MONITORING: Recommend weekly labs for first 2 weeks (electrolytes for refeeding syndrome risk), then every 2-4 weeks until nutritional rehabilitation. Specific monitoring intervals per [GUIDELINE]: [schedule]."
   - **If labs NOT documented AND diagnosis is MALNUTRITION ABSENT**: Use clinical reasoning with RAG: "No laboratory investigations documented in this encounter. Given the clinical presentation of adequate nutritional status with [supporting evidence: normal growth trajectory, adequate intake, well-nourished appearance], extensive lab work may not be clinically indicated at this time per [GUIDELINE]. Routine screening labs (CBC, CMP) could be considered at next well-child visit or if clinical status changes. If growth faltering develops, would recommend malnutrition-specific lab panel as outlined in [GUIDELINE]."
...
9. **SOCIAL CONTEXT - TEMPORAL (NEVER LEAVE EMPTY)**:
   - **If documented**: Extract temporal changes in circumstances with dates, describe intervention progression
   - **If NOT documented**: Use RAG guidelines: "Social determinants and family context not documented. Per [GUIDELINE], comprehensive social assessment for pediatric malnutrition should include: (1) Food security status and access; (2) Housing stability; (3) Caregiver resources and support systems; (4) Cultural/religious dietary practices; (5) Financial barriers to nutrition care; (6) WIC/SNAP enrollment; (7) Transportation for follow-up. Recommend social work consultation and standardized screening (e.g., [SCREENING TOOL]) to identify modifiable social barriers and connect family with community resources."
...
**CRITICAL RULES:**
- **NEVER LEAVE ANY FIELD EMPTY/BLANK/NULL**: Every field must have substantive content
- **DATA NOT DOCUMENTED**: Use RAG guidelines to write what SHOULD BE DONE (recommended assessments, monitoring, guidelines)
- **DATA IS DOCUMENTED**: Extract it AND enhance with RAG (compare to guidelines, add missing recommended elements)
- ANONYMIZE: Use "the patient", "the [age]-year-old", "the family"
...
- USE RAG GUIDELINES: For thresholds, criteria, and recommendations (reference guideline names)
- PRESERVE UNITS
- ALIGN WITH GROUND TRUTH: Support {label_context} with ALL temporal evidence

**REMEMBER**: The goal is comprehensive, guideline-based clinical documentation. Missing data is an opportunity to demonstrate expert knowledge by recommending evidence-based assessment and monitoring per RAG-retrieved guidelines.
...
**NEVER LEAVE FIELDS EMPTY**: If data NOT documented → use RAG to write guideline recommendations. If data IS documented → extract AND enhance with RAG guidelines. Capture ALL measurements with DATES. Calculate TRENDS. Identify assessment type (single/serial/longitudinal).
...
2. SYMPTOMS - TEMPORAL (NEVER EMPTY): **If documented**: Extract ALL with DATES, categories (GI/Systemic/Feeding/Functional), trajectory, quote. **If NOT**: Use RAG for recommendations: "Not documented. Per [GUIDELINE], assess: GI, systemic, feeding, functional symptoms with [interval] monitoring." **If documented**: Also compare with RAG for missing categories.

3. **GROWTH - TEMPORAL (NEVER EMPTY)**: **If documented**: Extract ALL with DATES, calculate TRENDS, describe pattern, interpret with RAG, identify missing parameters. **If NOT**: Use RAG: "Not documented. Per [GUIDELINE], obtain weight/height/BMI-for-age with percentiles/z-scores, serial measurements at [interval]."

4. **EXAM - TEMPORAL (NEVER EMPTY)**: **If documented**: Describe with dates, progression if serial, quote, interpret with RAG, note missing components. **If NOT**: Use RAG: "Not documented. Per [GUIDELINE], examine: appearance, muscle/fat, edema, skin/hair, growth plotting at each visit."

5. **INTAKE - TEMPORAL (NEVER EMPTY)**: **If documented**: Extract patterns with dates, timeframes. **If NOT**: Use RAG: "Not quantified. Per [GUIDELINE], assess: % needs, duration, foods, route, appetite. Recommend food diary, target [XX] kcal/kg/day per [GUIDELINE]."

6. **DIAGNOSIS (NEVER EMPTY)**: State consistent with ground truth. Synthesize evidence WITH TEMPORAL PATTERNS. Use RAG criteria. Specify severity/etiology. Reason with incomplete data using convergent evidence.

7. **LABS - TEMPORAL (NEVER EMPTY)**: **If documented**: Extract ALL with DATES, trends, interpret with RAG, note missing labs. **If NOT + malnutrition**: Use RAG: "Not documented. Per [GUIDELINE], obtain: CBC, CMP, albumin, vitamins. Serial monitoring: weekly x2, then q2-4wks." **If NOT + adequate**: "Not documented. Given adequate status, may not be indicated. Consider routine screening at well-child per [GUIDELINE]."
...
9. **SOCIAL - TEMPORAL (NEVER EMPTY)**: **If documented**: Extract changes with dates, intervention progression. **If NOT**: Use RAG: "Not documented. Per [GUIDELINE], assess: food security, housing, caregiver resources, cultural practices, financial barriers, WIC/SNAP, transportation. Recommend social work consult."

10. **INSIGHTS - TEMPORAL (NEVER EMPTY)**: Summarize with TEMPORAL INTEGRATION and RAG references. Prognosis with timeline. Decision points with dates. Risks with timeframes. Teaching. Pearls.

**RULES:**
- **NEVER LEAVE FIELDS EMPTY**: If not documented → RAG recommendations. If documented → extract + enhance with RAG
- ANONYMIZE
...
- USE RAG for criteria and recommendations
- ALIGN WITH GROUND TRUTH

**REMEMBER**: Missing data = opportunity to show guideline-based recommendations using RAG.
...
4. **FILL GAPS - PRIMARY OBJECTIVE**:
   - **EMPTY/NULL FIELDS**: If initial extraction has empty, null, blank, or "not documented" fields, THIS IS YOUR PRIMARY TASK - FILL THEM using RAG guidelines
   - **Symptoms field empty?** → Write: "Symptoms not documented in record. Per [RAG GUIDELINE], comprehensive symptom assessment should include: [detailed guideline-based symptom list with monitoring intervals]"
   - **Labs field empty?** → Write: "Labs not documented. Per [RAG GUIDELINE] for [severity] malnutrition, recommend: [detailed lab panel with specific tests and serial monitoring schedule from guidelines]"
   - **Growth field empty?** → Write: "Growth parameters not documented. Per [RAG GUIDELINE], obtain: [specific anthropometric measurements with percentile/z-score interpretation and monitoring frequency]"
   - **Exam field empty?** → Write: "Physical exam not documented. Per [RAG GUIDELINE], malnutrition exam should assess: [specific exam components from guidelines with serial documentation recommendations]"
   - **Intake field empty?** → Write: "Intake not quantified. Per [RAG GUIDELINE], assess: [specific intake assessment methods and target requirements from guidelines]"
   - **Social field empty?** → Write: "Social determinants not documented. Per [RAG GUIDELINE], assess: [specific social factors from guidelines with recommended interventions]"
   - **FIELDS WITH CONTENT**: Enhance by adding guideline-recommended elements not yet addressed, specify severity using RAG criteria, calculate missing trends, add monitoring schedules from guidelines
...
**CRITICAL PRINCIPLES:**
- **ABSOLUTE RULE: NO EMPTY FIELDS**: Every field must have substantive content in your refined output
- **EMPTY FIELDS ARE YOUR PRIMARY TASK**: Use RAG guidelines to fill with evidence-based recommendations
- **FILLED FIELDS ALSO NEED WORK**: Enhance with RAG guidelines, add missing guideline-recommended elements
- Preserve fidelity (don't remove correct documented data)
- Quote RAG guidelines explicitly (include guideline names)
- Flag discrepancies between initial extraction and ground truth
- EMBED FORWARD-THINKING with timelines and monitoring schedules from RAG
...
- **MINDSET**: You are demonstrating expert clinical knowledge by providing guideline-based recommendations for missing assessments
    ...
    "malnutrition": {
        ...
        "schema": {
            ...
            "clinical_symptoms_and_signs": {
                        "type": "string",
                        "description": "TEMPORAL SYMPTOM CAPTURE (NEVER LEAVE EMPTY): If documented: Extract ALL with ONSET DATES and PROGRESSION: (1) GI: vomiting, diarrhea, reflux, pain with frequency/severity/timing; (2) Systemic: fever, fatigue, irritability; (3) Feeding: appetite, refusal, satiety; (4) Functional: weakness, activity, development; (5) Trajectory: new/progressive/stable/improving/resolved. Quote exact descriptions. If NOT documented: Use RAG to write guideline-based symptom assessment recommendations with monitoring intervals. NEVER leave this field empty/null.",
                        "required": False
            },                
            "growth_and_anthropometrics": {
                "type": "string",
                "description": "TEMPORAL CAPTURE (NEVER LEAVE EMPTY): If documented: Extract ALL with DATES, calculate EXPLICIT TRENDS (absolute, %, rate, velocity, trajectory), describe pattern, use RAG for interpretation, note missing parameters. If NOT documented: Use RAG to write comprehensive anthropometric assessment recommendations with specific measurements, percentile/z-score interpretation, and serial monitoring frequency. NEVER leave empty.",
                "required": True
            },
            "physical_exam": {
                "type": "string",
                "description": "TEMPORAL (NEVER LEAVE EMPTY): If documented: Extract findings with dates, progression if serial, quote exactly, use RAG, note missing exam components. If NOT documented: Use RAG to write comprehensive exam recommendations covering appearance, muscle/fat assessment, edema, skin/hair, with serial documentation guidance. NEVER leave empty.",
                "required": True
            },
            "nutrition_and_intake": {
                "type": "string",
                "description": "TEMPORAL (NEVER LEAVE EMPTY): If documented: Extract patterns with dates and timeframes, relate to requirements. If NOT documented: Use RAG to write intake assessment recommendations with methods (food diary, recall), target requirements (kcal/kg, protein/kg), and quantification strategy. NEVER leave empty.",
                "required": True
            },
            ...
            "labs_and_screening": {
                "type": "string",
                "description": "TEMPORAL (NEVER LEAVE EMPTY): If documented: Extract ALL with DATES, describe TRENDS, use RAG, note missing labs. If NOT documented + malnutrition: Use RAG to write comprehensive lab recommendations (CBC, CMP, albumin, vitamins, etc.) WITH serial monitoring schedule (weekly x2, then q2-4wks). If NOT documented + adequate nutrition: Use RAG to explain appropriateness and recommend routine screening schedule. NEVER leave empty - always provide guideline-based content.",
                "required": False
            },
            "care_plan": {
                "type": "string",
            ...
            "social_context": {
                "type": "string",
                "description": "TEMPORAL (NEVER LEAVE EMPTY): If documented: Extract situation, barriers, changes with dates, intervention progression. If NOT documented: Use RAG to write social assessment recommendations covering food security, housing, caregiver resources, cultural practices, financial barriers, WIC/SNAP, transportation, with social work consultation recommendation. NEVER leave empty.",
                "required": False
            },
            "clinical_insights": {
                "type": "string",
